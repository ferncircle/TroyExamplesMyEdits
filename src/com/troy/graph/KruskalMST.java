 ...
 *spanning tree for a weighted, connected and undirected graph is a spanning tree with weight less than or equal to the 
 *weight of every other spanning tree. The weight of a spanning tree is the sum of weights given to each edge of the 
 *spanning tree.

A minimum spanning tree has (V – 1) edges where V is the number of vertices in the given graph.

 * Solution:
 * 1) Use disjoint set data structure to find set and merge two sets
 * 2) Sort edges in increasing order
 * 3) Pick edges from smallest to highest. For each edge find set for v1 and v2. If they are same then this edge will
 * 		introduce cycle, so ignore it. Otherwise merge these two sets
 */
...
public class KruskalMST {
    ...
    public List<Edge<Integer>> getMST(Graph<Integer> graph) {
        ...
        Collections.sort(allEdges, (a,b)->a.getWeight()<=b.getWeight()?-1:1);
